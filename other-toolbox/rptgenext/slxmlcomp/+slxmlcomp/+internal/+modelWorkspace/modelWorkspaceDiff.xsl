...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="MatFileEditScript">        
        ...
        <script type="text/javascript">
            <xsl:attribute name="src"><xsl:value-of select="$matlabroot"/>toolbox/rptgenext/slxmlcomp/+slxmlcomp/+internal/+modelWorkspace/modelWorkspaceDiff.js</xsl:attribute>
        </script>
        <style type="text/css">
            td, th {
                padding-left: 5px;
    ...
    <xsl:template name="comparelink">
        ...
        <xsl:variable name="iconfile" select="concat($matlabroot,'toolbox/shared/comparisons/private/vardiff.png')"/>
        <img class="compare">
            <xsl:attribute name="src"><xsl:value-of select="$iconfile"/></xsl:attribute>
            <xsl:attribute name="onclick">javascript:comparevar('<xsl:value-of select="$name"/>')</xsl:attribute>
